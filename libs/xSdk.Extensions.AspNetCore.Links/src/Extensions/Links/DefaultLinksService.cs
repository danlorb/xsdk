...
namespace xSdk.Extensions.Links
{
    public class DefaultLinksService : ILinksService
    {
        ...
        public async Task AddLinksAsync<TResource>(TResource linkContainer)
            where TResource : ILinkContainer
        {
                ...
                foreach (var policyName in currentRoute.LinksAttributes.Select(a => a.Policy))
                {
                    ...
                    if (controllerPolicy == null)
                        throw new InvalidOperationException(
                            $"Controller-registered Policy not defined: {policyName ?? "<default>"} Route: {currentRoute.RouteName}"
                        );
            ...
            if (typeAttributes.Any())
            {
                foreach (var policyName in typeAttributes.Select(a => a.Policy))
                {
                    ...
                    if (typePolicy == null)
                        throw new InvalidOperationException(
                            $"Type-registered policy not defined: {policyName ?? "<default>"} Type: {typeof(TResource).FullName}"
                        );
        ...
        public async Task AddLinksAsync<TResource>(TResource linkContainer, string policyName)
            where TResource : ILinkContainer
        {
            if (policyName == null)
            ...
            where TResource : ILinkContainer
        {
            ...
            {
                logger.LogWarning(
                    "Not all links were handled correctly for resource {ResourceType}. Unhandled requirements: {PendingRequirements}",
                    typeof(TResource).FullName,
                    ctx.PendingRequirements.ToArray()
                );
            }
